...
def main():
    ...
    for filename in os.listdir(image_dir):
        ...
        blurred_image = cv2.GaussianBlur(image, (0, 0), sigmaX=BLUR_SIGMA, sigmaY=BLUR_SIGMA)
        cv2.imwrite(os.path.join(aug_image_dir, base + f'_blur{BLUR_SIGMA}.jpg'), blurred_image)
        save_bbox(os.path.join(aug_label_dir, base + f'_blur{BLUR_SIGMA}.txt'), bboxes, image.shape)
